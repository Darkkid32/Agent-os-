...
import type { DashboardEnvelope, ToolsDTO } from '../../dashboard/api/types';

const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL ?? '/v1';

async function fetchTools(): Promise<DashboardEnvelope<ToolsDTO>> {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card">
        ...
        {data && data.items.length === 0 && (
          <p className="p-4 text-sm text-muted-foreground">No tools registered</p>
        )}
        {data &&
          data.items.map((tool) => (
            <div key={tool.id} className="border-b p-4 last:border-0">
              <div className="flex items-center justify-between mb-1">
                <p className="text-sm font-medium">{tool.name}</p>
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${tool.status === 'available' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}
                >
                  {tool.status}
                </span>
              </div>
